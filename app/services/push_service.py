...
class PushService:
    ...
    async def fix_outfit_style(self) -> int:
        replace_map = {
            "스포츠맨 (Jock)": "체대•운동남 (Jock)",
            "럼버잭 (Lumberjack)": "돌쇠남 (Lumberjack)",
            "프레피 (Preppy)": "아이비리그 스타일 (Preppy)",
            "독특한 패션 (Hipster)": "빈티지 감성 (Hipster)",
        }
        updated_count = 0
        async with self.db.get_connection() as conn:
            async with conn.cursor() as cur:
                ...
                for user_id, hashtags_json in rows:
                    try:
                        hashtags = json.loads(hashtags_json)
                        outfit = hashtags.get("outfitStyle", [])

                        new_outfit = [replace_map.get(x, x) for x in outfit]

                        if new_outfit != outfit:
                            hashtags["outfitStyle"] = new_outfit
                            await cur.execute(
                                "UPDATE users SET hashtags = %s WHERE id = %s",
                                (json.dumps(hashtags, ensure_ascii=False), user_id),
                            )
                            updated_count += 1
                    except Exception as e:
    ...
    async def fix_outfit_style_one(self, user_id: str = None) -> int:
        replace_map = {
            "스포츠맨 (Jock)": "체대•운동남 (Jock)",
            "럼버잭 (Lumberjack)": "돌쇠남 (Lumberjack)",
            "프레피 (Preppy)": "아이비리그 스타일 (Preppy)",
        ...
        updated_count = 0
        updated_users = []  # ✅ 업데이트된 user_id 기록
                ...
                for user_id, hashtags_json in rows:
                    try:
                        ...
                        if new_outfit != outfit:
                            ...
                            updated_count += 1
                            updated_users.append(user_id)  # ✅ 변경된 user_id 기록
        ...
        print(f"[INFO] Updated users: {updated_users}")  # ✅ 로그 출력
        return updated_count
